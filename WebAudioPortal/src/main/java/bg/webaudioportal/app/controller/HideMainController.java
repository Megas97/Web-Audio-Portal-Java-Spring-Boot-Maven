...
@RestController
@RequestMapping("/hideMain")
public class HideMainController {
	...
	public RedirectView  hideAudioResource(HttpServletRequest request, HttpServletResponse response, @PathVariable("fileName") String fileName, RedirectAttributes redirectAttributes) throws IOException {
		...
		if (user != null) {
			if (user.getActive() == 0) {
				hideMsg = "You cannot hide audio files until you activate your account!";
			} else {
				...
				if (file.exists()) {
					...
					if (currentAccount.equals(uploader)) {
						...
						hideMsg = "You successfully hid audio '" + name + "'";
					} else {
						hideMsg = "Only " + uploader + " can hide their audio files!";
					}
				}
			...
			hideMsg = "Please log in to be able to hide audio files!";
		}
		redirectAttributes.addFlashAttribute("actionMsg", hideMsg);
